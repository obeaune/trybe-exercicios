...
const getPlanet = () => {
  setTimeout(() => {
    ...
    // poderia ser também com o setTimeout pro console.log (mais simples, creio eu):
...
const sendMarsTemperature = (success, error) => {
  ...
  const didMessageSent = Math.random() <= 0.6;
  setTimeout(() => {
    if (didMessageSent) success(currentTemp);
...
test('Testar se as letras ficam maiúsculas', (done) => {
  uppercase('maior', (str) => {
    try {
      expect(str).toBe('MAIOR');
      done();
    } catch (error) {
      done(error);
    }
